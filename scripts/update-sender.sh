#!/bin/bash
# update-sender.sh - обновление sender компонента swapip
...
usage() {
    ...
    echo ""
    echo "Описание:"
    echo "  Автоматически проверяет и устанавливает обновления для sender компонента."
...
check_update_needed() {
    log_info "Проверка текущей версии sender..."
    ...
    log_info "Полученные данные релиза (первые 200 символов): ${release_data:0:200}"
    ...
    if [ $? -ne 0 ] || [ -z "$LATEST_VERSION" ]; then
        ...
        log_info "Код возврата get_version_from_release: $?"
        return 1
    fi
    ...
    if [ "$CURRENT_VERSION" = "0.0.0" ]; then
        log_info "Sender не установлен. Будет установлена последняя версия."
        NEED_UPDATE=true
        return 0
    fi
...
download_new_version() {
    # Проверка что версия определена
    if [ -z "$LATEST_VERSION" ] || [ "$LATEST_VERSION" = "0.0.0" ]; then
        log_error "Версия для загрузки не определена (LATEST_VERSION=$LATEST_VERSION)"
        return 1
    fi
    
    local version=$LATEST_VERSION
    log_info "Получена версия для загрузки: $LATEST_VERSION (тег: $RELEASE_TAG)"
    ...
    # log_info "$release_data"  # Комментируем вывод полного JSON для читаемости
...
# Основная функция
main() {
    log_info "Начало работы скрипта обновления sender..."
    ...
    if ! create_directories; then
        log_error "Ошибка при созданы директории"
        exit 1
    fi
    ...
    send_notification "Sender обновлен" "Версия $CURRENT_VERSION -> $updated_version" "success" 2>/dev/null || true
