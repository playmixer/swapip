#!/bin/bash
# rollback-recepient.sh - откат recepient компонента к предыдущей версии
...
# Конфигурация
COMPONENT="recepient"
SERVICE_NAME="swapip-recepient"
INSTALL_PATH="/opt/swapip/recepient"
BACKUP_DIR="/opt/swapip/backups"
...
usage() {
    ...
    echo ""
    echo "Описание:"
    echo "  Выполняет откат recepient компонента к предыдущей версии из резервной копии."
    echo ""
    echo "Пример:"
    echo "  $0 --list"
    echo "  $0 --latest"
    echo "  $0 --backup /opt/swapip/backups/recepient_1.0.0_20250101_120000.tar.gz"
}
...
check_installation() {
    if [ ! -d "$INSTALL_PATH" ]; then
        log_error "Recepient не установлен в $INSTALL_PATH"
        exit 1
    fi
...
perform_rollback() {
    ...
    if systemctl is-active --quiet "$SERVICE_NAME"; then
        ...
        log_success "Служба остановлена"
    else
        log_info "Служба не запущена"
    fi
    ...
    if systemctl start "$SERVICE_NAME"; then
        log_success "Служба успешно запущена"
        
        # Проверка статуса
        sleep 2
        if systemctl is-active --quiet "$SERVICE_NAME"; then
            log_success "Служба работает корректно"
        else
            log_error "Служба не запустилась после отката"
            return 1
        fi
    else
...
rollback_to_specific() {
    ...
    if [ "$DRY_RUN" = false ]; then
        echo ""
        echo "Вы собираетесь выполнить откат recepient:"
        echo "  Из: $backup_file"
        echo "  В: $INSTALL_PATH"
        echo ""
...
# Основная функция
main() {
    log_info "Начало работы скрипта отката recepient..."
    ...
    if [ $? -eq 0 ] && [ "$ACTION" != "list" ]; then
        log_success "=========================================="
        log_success "Откат recepient выполнен успешно!"
        log_success ""
        log_success "Текущая версия: $(get_current_version "$COMPONENT")"
        log_success ""
        log_success "Команды управления:"
        log_success "  Проверка статуса: systemctl status $SERVICE_NAME"
        log_success "  Проверка версии: $INSTALL_PATH/$COMPONENT --version"
        log_success ""
